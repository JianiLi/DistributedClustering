...
def generateData_mnist(train_data, test_data, tr_split_len, te_split_len, number):
    ...
    test_loader_no = DataLoader(dataset=test_data_no, batch_size=64, shuffle=True)
...
def generateData_synthetic_digits(remaining_tr, remaining_te, tr_split_len, te_split_len):
    ...
    test_loader_no = DataLoader(part_te, batch_size=128, shuffle=True)
    ...
    if rule == "loss":
        ...
        for l in range(0, N):
            if Accumulated_Loss[ego_k, l] <= Accumulated_Loss[ego_k, ego_k]:
                ...
                Weight[l] = weight
        Weight[ego_k] = 1 - sum(Weight[:ego_k]) - sum(Weight[ego_k + 1:])
...
def cooperation(A, A_last, Batch_X, Batch_Y, Accumulated_Loss, rule, attacker, Weight, epoch):
    Parameters_last = []
    Parameters_exchange = []
    N = len(A)
    ...
    return A, Accumulated_Loss, Weight
...
def run(rule, epochs):
    ...
    for epoch in range(epochs):
        ...
        try:
            while True:
                ...
                for k in range(0, N):
                    ...
                    if k in attacker:
                        continue
                    # 5 agents, get access to 1, 1/2, 1/3, 1/5 data, so their models have different accuracy
                    if k % 5 == 1:
                ...
                A, Accumulated_Loss, Weight = cooperation(A, A_last, Batch_X, Batch_Y, Accumulated_Loss, rule, attacker,
                                                          Weight, epoch)
                # print(Accumulated_Loss)
        ...
        try:
            print(
                'Total Average Train Loss: {:.6f}, Train Acc: {:.6f}'.format(
        ...
        if epoch % 10 == 0 or epoch == epochs - 1:
            if attacker_num == 0:
                ...
                np.save('results/Weight_%s.npy' % rule, Weight)
...
if __name__ == '__main__':
    epochs = 300
    for rule in ["loss", "no-cooperation", "distance", "average"]:
        print("rule: ", rule)
        run(rule, epochs)
